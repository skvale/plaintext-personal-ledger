<script lang="ts">
  ...
  function suggestAccount(description: string): string {
    ...
    const allKeywords = [...(_props.data.userKeywords ?? []), ...BUILT_IN_KEYWORDS];
    for (const entry of allKeywords) {
      if (entry.keywords.some((kw: string) => lower.includes(kw))) return entry.account;
    }
    return '';
  }

  const BUILT_IN_KEYWORDS = [
    { keywords: ['restaurant', 'bistro', 'grill', 'diner', 'izakaya', 'trattoria', 'brasserie'], account: 'expenses:food:dining' },
    { keywords: ['pizza', 'sushi', 'taco', 'burger', 'ramen', 'pho', 'bbq', 'wok', 'noodle'], account: 'expenses:food:dining' },
    { keywords: ['chipotle', 'mcdonalds', 'wendys', 'chick-fil-a', 'subway', 'panera', 'panda express'], account: 'expenses:food:dining' },
    { keywords: ['doordash', 'grubhub', 'ubereats', 'postmates', 'seamless'], account: 'expenses:food:dining' },
    { keywords: ['bar', 'pub', 'brewery', 'tavern', 'taproom', 'winery'], account: 'expenses:food:dining' },
    { keywords: ['bakery', 'deli', 'bagel', 'smoothie', 'juice'], account: 'expenses:food:dining' },
    { keywords: ['starbucks', 'dunkin', 'cafe', 'coffee', 'peets', 'tim hortons'], account: 'expenses:food:coffee' },
    { keywords: ['grocery', 'grocer', 'supermarket'], account: 'expenses:food:groceries' },
    { keywords: ['whole foods', 'trader joe', 'kroger', 'safeway', 'aldi', 'publix', 'wegmans', 'heb'], account: 'expenses:food:groceries' },
    { keywords: ['costco', "sam's club", "bj's"], account: 'expenses:food:groceries' },
    { keywords: ['amazon', 'target', 'walmart', 'bestbuy', 'best buy', 'ikea', 'home depot', 'lowes'], account: 'expenses:shopping' },
    { keywords: ['store', 'shop', 'mall', 'outlet', 'marketplace', 'hilldale'], account: 'expenses:shopping' },
    { keywords: ['ebay', 'etsy', 'wayfair', 'overstock'], account: 'expenses:shopping' },
    { keywords: ['netflix', 'spotify', 'hulu', 'disney', 'hbo', 'peacock', 'paramount', 'youtube'], account: 'expenses:entertainment:streaming' },
    { keywords: ['apple', 'itunes', 'app store', 'google play'], account: 'expenses:entertainment:subscriptions' },
    { keywords: ['cinema', 'theater', 'theatre', 'movie', 'amc', 'regal'], account: 'expenses:entertainment' },
    { keywords: ['ticketmaster', 'stubhub', 'eventbrite', 'livenation', 'concert', 'ticket'], account: 'expenses:entertainment' },
    { keywords: ['xbox', 'playstation', 'steam', 'nintendo'], account: 'expenses:entertainment' },
    { keywords: ['shell', 'exxon', 'chevron', 'bp', 'mobil', 'sunoco', 'gas station', 'fuel'], account: 'expenses:transportation:gas' },
    { keywords: ['uber', 'lyft', 'taxi', 'cab', 'rideshare'], account: 'expenses:transportation' },
    { keywords: ['parking', 'parkwhiz', 'spothero'], account: 'expenses:transportation:parking' },
    { keywords: ['toll', 'ezpass', 'fastrak'], account: 'expenses:transportation:tolls' },
    { keywords: ['airline', 'flight', 'delta', 'united', 'southwest', 'american air', 'jetblue', 'frontier', 'spirit'], account: 'expenses:travel:flights' },
    { keywords: ['hotel', 'airbnb', 'vrbo', 'marriott', 'hilton', 'hyatt', 'motel'], account: 'expenses:travel:lodging' },
    { keywords: ['pharmacy', 'cvs', 'walgreens', 'rite aid', 'prescription'], account: 'expenses:health' },
    { keywords: ['doctor', 'hospital', 'clinic', 'medical', 'urgent care', 'lab', 'quest diagnostics'], account: 'expenses:health' },
    { keywords: ['dental', 'dentist', 'orthodontist'], account: 'expenses:health:dental' },
    { keywords: ['vision', 'optometrist', 'eye', 'lenscrafters'], account: 'expenses:health:vision' },
    { keywords: ['electric', 'power', 'energy', 'pge', 'duke energy', 'con edison'], account: 'expenses:housing:utilities' },
    { keywords: ['water', 'sewer', 'garbage', 'waste', 'trash'], account: 'expenses:housing:utilities' },
    { keywords: ['comcast', 'xfinity', 'spectrum', 'att', 'verizon fios', 'internet'], account: 'expenses:housing:utilities' },
    { keywords: ['rent', 'landlord', 'lease'], account: 'expenses:housing:rent' },
    { keywords: ['mortgage'], account: 'expenses:housing:mortgage' },
    { keywords: ['insurance', 'geico', 'progressive', 'state farm', 'allstate', 'usaa'], account: 'expenses:insurance' },
    { keywords: ['t-mobile', 'tmobile', 'verizon wireless', 'att wireless', 'mint mobile', 'visible'], account: 'expenses:phone' },
    { keywords: ['gym', 'fitness', 'planet fitness', 'equinox', 'orangetheory', 'crossfit', 'ymca', 'peloton'], account: 'expenses:fitness' },
    { keywords: ['tuition', 'school', 'university', 'college', 'udemy', 'coursera', 'skillshare'], account: 'expenses:education' },
    { keywords: ['book', 'kindle', 'audible'], account: 'expenses:education:books' },
    { keywords: ['vet', 'veterinary', 'petco', 'petsmart', 'chewy'], account: 'expenses:pets' },
    { keywords: ['haircut', 'salon', 'barber', 'spa', 'massage', 'nail'], account: 'expenses:personal-care' },
    { keywords: ['dry clean', 'laundry', 'cleaners'], account: 'expenses:personal-care' },
    { keywords: ['donation', 'charity', 'nonprofit', 'gofundme', 'patreon'], account: 'expenses:charity' },
    { keywords: ['gift', 'present', 'flowers', 'hallmark'], account: 'expenses:gifts' },
  ];
  ...
  function buildHeader(original: string, directives: Record<string, string>): string {
    let h = original;
  ...
  function parseExtraPostings(header: string): { n: number; account: string; amount: string }[] {
  ...
  const OPTIONAL_DIRECTIVES = [
    { key: 'decimal-mark',       label: 'Decimal mark',       help: 'Decimal point character (. or ,). Use , for European-style CSVs.' },
    { key: 'separator',          label: 'Separator',          help: 'CSV field separator character. Default is comma; use \\t for TSV.' },
    { key: 'encoding',           label: 'Encoding',           help: 'File encoding of the CSV, e.g. utf-8 or latin1.' },
    { key: 'timezone',           label: 'Timezone',           help: 'Timezone for interpreting dates, e.g. America/New_York.' },
    { key: 'newest-first',       label: 'Newest first',       help: 'Set to true if the CSV lists newest transactions at the top.' },
    { key: 'intra-day-reversed', label: 'Intra-day reversed', help: 'Set to true to reverse the order of transactions sharing the same date.' },
    { key: 'balance-type',       label: 'Balance type',       help: 'Balance assertion style to generate: =, ==, =*, or ==*.' },
    { key: 'source',             label: 'Source',             help: 'Override the source account (account1) for all transactions.' },
    { key: 'archive',            label: 'Archive',            help: 'Filesystem path to move the CSV to after a successful import.' },
  ] as const;

  function parseExtraDirectives(header: string): { key: string; value: string }[] {
    return OPTIONAL_DIRECTIVES
      .map(d => ({ key: d.key, value: parseDirective(header, d.key) }))
      .filter(d => d.value !== '');
  }
  ...
  const availableDirectives = $derived(
    OPTIONAL_DIRECTIVES.filter(d => !extraDirectives.some(e => e.key === d.key))
  );
  ...
  const currentHeader = $derived(
    buildHeader(baseHeader, {
      ...
      ...Object.fromEntries(extraPostings.flatMap(p => [
  ...
  const HEADER_KEYS = new Set([
    'skip', 'fields', 'date-format', 'currency', 'decimal-mark', 'separator',
    'encoding', 'timezone', 'newest-first', 'intra-day-reversed', 'balance-type',
    'source', 'archive', 'description',
  ]);
  function isHeaderDirective(line: string): boolean {
    const key = line.split(/\s+/)[0];
    return HEADER_KEYS.has(key) || /^account\d+$/.test(key) || /^amount\d+$/.test(key);
  }
  function parseRulesClient(content: string): { header: string; items: RulesItem[] } {
    ...
    while (i < lines.length) {
      const line = lines[i];
      if (inHeader) {
        if (line.startsWith('if ') || line.startsWith('include ')) { inHeader = false; continue; }
        headerLines.push(line); i++; continue;
      }
      if (isHeaderDirective(line)) { headerLines.push(line); i++; continue; }
      if (line.startsWith('if ')) {
        ...
        ruleItems.push({ type: 'rule', id: `r${i}`, patterns, account, assignments });
        i = j;
      } else if (line.startsWith('include ')) {
        ruleItems.push({ type: 'include', id: `inc-${i}`, path: line.slice(8).trim() });
        i++;
      } else if (/^; ===/.test(line)) {
  ...
  const allAccountsWithPending = $derived([...new Set([...(data.accounts as string[]), ...pendingAccounts])].sort());
  ...
  function addRule() {
    items = [...items, { type: 'rule', id: `new-${uid++}`, patterns: '', account: '', assignments: [{ key: 'account2', value: '' }] }];
    mark();
  }
  ...
  const activeDragItem = $derived(activeDragId ? items.find(x => x.type === 'rule' && x.id === activeDragId) as RulesItem & { type: 'rule' } | undefined : undefined);
  ...
  function updateRule(id: string, field: 'patterns' | 'account', value: string) {
    items = items.map((x) => {
  ...
  function updateAssignment(ruleId: string, assignIdx: number, field: 'key' | 'value', val: string) {
    items = items.map((x) => {
  ...
  function addAssignment(ruleId: string) {
    items = items.map((x) => {
      ...
      const nums = assigns.map(a => { const m = a.key.match(/^account(\d+)$/); return m ? +m[1] : 0; });
  ...
  const sortableIds = $derived(items.filter(x => x.type !== 'raw').map(x => x.id));
  ...
  function createFromCsvEnhance() {
    return async ({ result, update }: any) => {
      if (result.type === 'success' && result.data?.createdFromCsv) {
        csvFileSelected = '';
        await update();
        goto(`/mappings?file=${encodeURIComponent(result.data.createdFromCsv)}`);
      } else if (result.type === 'failure') {
...
<LearningBanner id="mappings" title="Auto-sorting imports">
  Import mappings automatically sort your transactions into the right categories. For example: "anything with
  WHOLE FOODS in the description goes to expenses:food:groceries." The more mappings you add, the less manual
  sorting you'll need to do after each import.
</LearningBanner>
...
<div class="space-y-5">
  ...
  <div class="flex items-center justify-between">
    ...
    <div class="flex items-center gap-3">
      {#if dirty}
        ...
        <button
          type="button"
          onclick={() => document.querySelector<HTMLFormElement>('#mappingsForm')?.requestSubmit()}
          disabled={saving}
      ...
      {#if data.active}
        <button
          ...
          class="rounded-md px-2.5 py-1 text-sm font-medium transition-colors
            {showRaw ? 'bg-slate-700 text-slate-100' : 'text-slate-100 hover:bg-slate-800 hover:text-slate-100'}"
        >
  ...
  <div class="flex flex-wrap items-center gap-1">
    {#each data.rulesFiles as file}
      {#if renamingFile === file}
        <form method="POST" action="?/rename" use:enhance={renameEnhance(file)} class="flex items-center gap-1">
          <input
            type="hidden" name="oldName" value={file}
          />
          <input
            type="text"
            name="newName"
            ...
            autofocus={true}
            onkeydown={(e) => { if (e.key === 'Escape') { renamingFile = null; renameInput = ''; renameError = ''; } }}
          ...
          <button type="submit" class="rounded-md bg-blue-300/10 px-2 py-1 text-xs text-blue-500 hover:bg-blue-300/20">Save</button>
          <button type="button" onclick={() => { renamingFile = null; renameInput = ''; renameError = ''; }} class="text-slate-400 hover:text-slate-200 text-xs px-1">✕</button>
        </form>
        {#if renameError}
      ...
      {:else}
        ...
        >
          ...
          <button
            type="button"
            onclick={() => { renamingFile = file; renameInput = file.replace(/\.rules$/, ''); renameError = ''; }}
    ...
    {#if creatingFile}
      <form method="POST" action="?/create" use:enhance={createEnhance} class="flex items-center gap-1">
        <input
          type="text"
          name="name"
          ...
          autofocus
          onkeydown={(e) => { if (e.key === 'Escape') { creatingFile = false; newFileName = ''; createError = ''; } }}
          class="rounded-md border border-slate-400 bg-slate-900 px-2 py-1 text-sm text-slate-100 outline-none focus:border-blue-300 w-36"
        />
        <span class="text-xs text-slate-400">.rules</span>
        <button type="submit" class="rounded-md bg-blue-300/10 px-2 py-1 text-xs text-blue-500 hover:bg-blue-300/20">Create</button>
        <button type="button" onclick={() => { creatingFile = false; newFileName = ''; createError = ''; }} class="text-slate-400 hover:text-slate-200 text-xs px-1">✕</button>
      </form>
      {#if createError}
      ...
      >
        ...
        <input
          ...
          onchange={async (e) => {
            ...
            fd.set('csvFile', file);
            const res = await fetch('?/createFromCsv', { method: 'POST', body: fd });
  ...
  {#if data.active && showRaw}
    <div class="space-y-3">
      <textarea
        ...
        onblur={() => { rawFocused = false; rawTextLocal = serializeRules(currentHeader, items); }}
        spellcheck="false"
        rows="20"
  ...
  {#if data.active}
    ...
    {#if data.parsed.header.trim() || data.csvDefaults}
      <div class="overflow-hidden rounded-lg border border-slate-400 bg-slate-900">
        <div class="border-b border-slate-400 px-4 py-2 flex items-center justify-between gap-3">
          <span class="text-xs font-semibold tracking-wide text-slate-100">Directives</span>
            ...
            <div class="flex items-center gap-2">
              ...
              <select
                ...
                onchange={async (e) => {
                  ...
                  if (!d) { detectError = 'No results'; return; }
                  // Additive: only fill empty directives
                  if (!skip && d.skip) { skip = d.skip; mark(); }
                  if (!fields && d.fields) { fields = d.fields; mark(); }
                  if (!dateFormat && d.dateFormat) { dateFormat = d.dateFormat; mark(); }
                  if (!currency && d.currency) { currency = d.currency; mark(); }
                  if (!defaultAccount2 && d.account2) { defaultAccount2 = d.account2; mark(); }
                  // Add mappings for new descriptions
                  if (d.descriptions?.length) {
                    const existingPatterns = new Set(
                      items.filter(x => x.type === 'rule').map(x => x.patterns?.toUpperCase())
                    );
                    for (const desc of d.descriptions) {
                      if (!existingPatterns.has(desc.toUpperCase())) {
                        const suggested = suggestAccount(desc) || defaultAccount2 || 'expenses:unknown';
                        items = [...items, {
                          type: 'rule',
                          id: `detect-${uid++}`,
                          patterns: desc,
                          account: suggested,
                          assignments: [{ key: 'account2', value: suggested }]
                        }];
                      }
                    }
                    mark();
              ...
              >
                <option value="">{detecting ? 'Detecting…' : 'Detect from CSV…'}</option>
        ...
        <div class="grid grid-cols-2 gap-x-6 gap-y-3 px-4 py-3">
          {#each [
            ['skip', 'Skip rows', skip, (v: string) => { skip = v; mark(); }, 'Number of header rows to skip at the top of the CSV. Most bank exports have 1 header row.'],
            ['fields', 'Fields', fields, (v: string) => { fields = v; mark(); }, 'Maps CSV columns to transaction fields — order matters, it must match the column order in your CSV. Common fields: date, description, amount, amount-in, amount-out, balance. Use a blank name to skip a column.'],
            ['date-format', 'Date format', dateFormat, (v: string) => { dateFormat = v; mark(); }, 'How dates appear in this CSV. Examples: %m/%d/%Y (12/31/2025), %Y-%m-%d (2025-12-31), %d/%m/%Y (31/12/2025).'],
            ['currency', 'Currency', currency, (v: string) => { currency = v; mark(); }, 'Currency symbol to prepend to amounts, e.g. $ or £ or EUR.'],
            ['description', 'Description', description, (v: string) => { description = v; mark(); }, 'Static description applied to all imported transactions from this CSV.'],
          ] as [key, label, value, setter, help]}
              ...
              <div class="flex items-center gap-3">
                <div class="w-28 shrink-0 flex items-center gap-1.5">
                  ...
                  {#if learningEnabled && help}
                    <Popover.Root>
                      <Popover.Trigger class="text-slate-100 hover:text-slate-100 transition-colors">
                        <svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><path d="M12 17h.01"/></svg>
                      </Popover.Trigger>
                      <Popover.Portal>
                        <Popover.Content
                          side="top"
                ...
                <input
                  ...
                  oninput={(e) => (setter as any)((e.target as HTMLInputElement).value)}
                  ...
                  <Popover.Root>
                    ...
                    <Popover.Portal>
                      <Popover.Content
                        ...
                        sideOffset={6}
                        class="z-50 max-w-xs rounded-lg border border-blue-300/20 bg-blue-300/5 px-3 py-2 text-sm leading-relaxed text-blue-500 shadow-xl backdrop-blur-sm"
                      >The first posting's account — usually the bank or credit card this CSV belongs to, e.g. assets:checking or liabilities:visa.</Popover.Content>
                    </Popover.Portal>
                  </Popover.Root>
                {/if}
              </div>
            ...
            <div class="flex items-center gap-3">
                  ...
                  <Popover.Root>
                    ...
                    <Popover.Portal>
                      <Popover.Content
                        ...
                        sideOffset={6}
                        class="z-50 max-w-xs rounded-lg border border-blue-300/20 bg-blue-300/5 px-3 py-2 text-sm leading-relaxed text-blue-500 shadow-xl backdrop-blur-sm"
                      >The second posting's default account — used when no mapping rule matches. Typically "expenses:unknown" so unmatched transactions show up in triage.</Popover.Content>
                    </Popover.Portal>
                  </Popover.Root>
                {/if}
              </div>
              <div class="flex-1">
                <Combobox
                  ...
                  onchange={(v) => { defaultAccount2 = v; mark(); }}
                  oncreate={trackAccount}
          ...
          {#each extraPostings as posting, idx}
            <div class="col-span-2 flex items-center gap-2">
              <span class="w-28 shrink-0 text-xs text-slate-100">Split {idx + 1}</span>
              <div class="flex-1">
                <Combobox
                  items={allAccounts}
                  value={posting.account}
                  onchange={(v) => { extraPostings[idx].account = v; mark(); }}
                  oncreate={trackAccount}
                  placeholder="account"
                  inputClass="w-full border border-slate-300 rounded-md bg-slate-900 px-2 py-1 pr-6 font-mono text-sm text-slate-100 placeholder:text-slate-100 focus:border-blue-300 outline-none"
                />
              ...
              <input
                ...
                oninput={(e) => { extraPostings[idx].amount = (e.target as HTMLInputElement).value; mark(); }}
              ...
              />
              <button type="button" onclick={() => removeExtraPosting(idx)} class="flex h-8 w-8 shrink-0 items-center justify-center rounded border border-rose-500/30 text-rose-400 transition-colors hover:border-rose-500/60 hover:bg-rose-500/10"><svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M18 6 6 18M6 6l12 12"/></svg></button>
            </div>
          {/each}

          {#if extraPostings.length < 7}
            <div class="col-span-2 pt-1">
              <button type="button" onclick={addExtraPosting} class="text-xs text-slate-400 hover:text-slate-200 transition-colors">+ Add split</button>
            </div>
          {/if}
          ...
          {#each extraDirectives as ed, idx}
            <div class="flex items-center gap-3">
              <div class="w-28 shrink-0">
                <span class="text-xs text-slate-100">{OPTIONAL_DIRECTIVES.find(d => d.key === ed.key)?.label ?? ed.key}</span>
              </div>
              <input
                type="text"
                value={ed.value}
                oninput={(e) => { extraDirectives[idx].value = (e.target as HTMLInputElement).value; mark(); }}
                class="flex-1 rounded-md border border-slate-300 bg-slate-900 px-2 py-1 font-mono text-sm text-slate-100 outline-none focus:border-blue-300"
              />
              <button type="button" onclick={() => removeExtraDirective(idx)} class="flex h-8 w-8 shrink-0 items-center justify-center rounded border border-rose-500/30 text-rose-400 transition-colors hover:border-rose-500/60 hover:bg-rose-500/10"><svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M18 6 6 18M6 6l12 12"/></svg></button>
            </div>
          {/each}
    ...
    >
      ...
      <input type="hidden" name="payload" value={JSON.stringify({ header: currentHeader, items })} />
      ...
      <DndContext {onDragStart} {onDragEnd}>
        <SortableContext items={sortableIds.map(id => ({ id }))}>
          <div class="space-y-0">
            {#each items as item (item.id)}
              ...
              {:else if item.type === 'include'}
                <div class="flex items-center gap-2 rounded-md border border-slate-400 bg-slate-900/50 px-3 py-2 mb-1.5">
                  <span class="text-xs text-slate-100">include</span>
                  <span class="font-mono text-sm text-slate-100">{item.path}</span>
                </div>
              {:else if item.type === 'raw'}
              ...
              {:else}
                <RuleSortableItem
                  id={item.id}
                  item={{ patterns: item.patterns ?? '', account: item.account ?? '', assignments: item.assignments ?? [] }}
                  accounts={allAccounts}
                  onupdate={(field, value) => updateRule(item.id, field, value)}
                  onremove={() => removeRule(item.id)}
                  onupdateassignment={(idx, field, val) => updateAssignment(item.id, idx, field, val)}
        ...
        <DragOverlay>
          {#if activeDragItem}
            <div class="flex items-center gap-2 rounded-md border border-slate-600 bg-slate-800 px-3 py-2 shadow-lg">
              <span class="text-slate-100 select-none">⠿</span>
              <span class="font-mono text-sm text-amber-400/80">{activeDragItem.patterns}</span>
              <span class="text-sm text-slate-100">→</span>
              <span class="font-mono text-sm text-emerald-400/80">{activeDragItem.account}</span>
            </div>
          {/if}
        </DragOverlay>
      </DndContext>
  ...
  {:else}
    <p class="text-sm text-slate-400">No .rules files yet. Use <span class="font-medium text-slate-300">+ New file</span> above to create one.</p>
  {/if}
