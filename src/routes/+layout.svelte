<script lang="ts">
  ...
  const NAV_REGISTRY: Record<
    string,
    { href: string; label: string; icon: string; key?: string }
  > = {
    dashboard: {
      href: "/dashboard",
      label: "Dashboard",
  ...
  $effect(() => {
    if (data?.settings?.sidebar?.items?.map) {
      dndItems = data.settings.sidebar.items.map((c, i) => ({
        id: navItemId(c, i),
        config: c,
      }));
    }
  ...
  const activeNavItem = $derived(
    activeNavId
      ? navItems[dndItems.findIndex((d) => d.id === activeNavId)]
      : null,
  );
  ...
  const navShortcutItems = $derived(
    dndItems
      .map((d) =>
        typeof d.config === "string" ? NAV_REGISTRY[d.config] : null,
      )
  ...
  const glossaryTerms = [
    {
      term: "Transaction",
      def: "A journal entry describing a movement of money between accounts. Each transaction has a date, description, and one or more postings.",
    },
    {
      term: "Posting",
      def: "An individual account change within a transaction. Each posting names an account and an amount. All postings in a transaction sum to zero. hledger requires at least two postings per transaction.",
    },
    {
      term: "Account",
      def: "A named category you track, like assets:bank:checking or expenses:food. Colons separate hierarchy levels. The five account types in hledger are assets, liabilities, income, expenses, and equity.",
    },
    {
      term: "Register",
      def: "The plain text journal file (.journal) containing all your transactions. hledger reads this file as its single source of truth. Human-readable, version-controlled, and directly editable.",
    },
    {
      term: "Double-Entry",
      def: "The core principle: every transaction affects at least two accounts, and all postings sum to zero. hledger enforces this — if a transaction doesn't balance, it's rejected.",
    },
    {
      term: "Balance Sheet",
      def: "A hledger report (balancesheet command) showing what you own (assets) vs. what you owe (liabilities) at a point in time. Net worth is the difference.",
    },
    {
      term: "P&L / Income Statement",
      def: "A hledger report (incomestatement command) summarizing income earned and expenses incurred over a period. Shows whether you're saving or spending beyond your means.",
    },
    {
      term: "Reconcile",
      def: "Matching your recorded transactions against bank or credit card statements. In hledger, you mark postings as cleared (*) or uncleared, then use register and check commands to find discrepancies.",
    },
    {
      term: "Net Worth",
      def: "Total assets minus total liabilities. What you own minus what you owe. hledger computes this from your balance sheet.",
    },
    {
      term: "Forecast",
      def: "hledger's --forecast flag generates future transactions from periodic rules. Shows projected balances based on recurring entries.",
    },
    {
      term: "Budget",
      def: "A spending target assigned to an account. hledger's budget report compares planned vs actual amounts, flagging overspend.",
    },
    {
      term: "Vendor",
      def: "The payee or description on a transaction — who you paid or who paid you. Not a hledger term; used in this app for grouping transactions by description.",
    },
    {
      term: "Recurring",
      def: "A transaction pattern that repeats on a schedule (rent, subscriptions). Defined as periodic rules that hledger uses with --forecast to generate future entries.",
    },
    {
      term: "Cost Basis",
      def: "What you originally paid for an investment. hledger tracks it via @ cost notation in postings and P price directives. View with the -B flag (cost) vs -V (market value).",
    },
    {
      term: "Unrealized Gain",
      def: "The difference between current market value (-V) and original cost (-B) of investments you still hold. hledger computes this automatically without manual mark-to-market entries.",
    },
  ];
  ...
  const pageTerms: Record<string, string[]> = {
    ...
    "/portfolio": ["Cost Basis", "Unrealized Gain", "Account"],
    ...
    "/welcome": ["Register", "Transaction", "Account", "Double-Entry"],
    "/tx": [
      "Transaction",
      "Posting",
      "Double-Entry",
...
<div class="flex h-screen overflow-hidden bg-slate-950">
  ...
  >
    ...
    <!-- Nav -->
    <nav
      class="sidebar-nav-scroll flex flex-1 flex-col gap-0.5 overflow-y-auto p-2"
    >
      <DndContext onDragStart={onNavDragStart} onDragEnd={onNavDragEnd}>
        ...
        <DragOverlay dropAnimation={null}>
          {#if activeNavItem && activeNavItem.kind === "link"}
            <div
              class="flex items-center gap-2.5 rounded-md bg-slate-900 border border-slate-300 px-3 py-2 text-sm font-medium text-slate-100 shadow-md shadow-black/30"
            >
          ...
          {:else if activeNavItem && activeNavItem.kind === "section"}
            <div
              class="px-3 pb-0.5 text-xs font-semibold uppercase tracking-widest text-slate-100"
            >
              {activeNavItem.label}
            </div>
          {/if}
    ...
    <div
      class="p-3"
      style="border-top: 1px solid transparent; border-image: linear-gradient(to right, transparent, var(--color-slate-700), transparent) 1;"
    >
    ...
    >
      ...
      >
        ...
        <kbd
          class="ml-auto rounded border border-slate-600 bg-slate-800 px-1.5 py-0.5 font-mono text-xs text-slate-100"
          >?</kbd
        >
      </button>
    </div>
  ...
  <main class="flex min-w-0 flex-1 flex-col overflow-hidden">
    <!-- Top bar -->
    <div
      class="flex shrink-0 items-center gap-3 border-b border-slate-400 px-4 py-2"
    >
      <!-- Hamburger (mobile) -->
      <button
      ...
      <form onsubmit={handleSearch} class="flex flex-1 max-w-xs items-center">
        <div class="relative flex-1">
          <span
            class="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-slate-100"
            >/</span
          >
          <input
            bind:this={searchEl}
    ...
    <div bind:this={contentEl} class="flex-1 overflow-y-auto">
      <div class="mx-auto max-w-5xl px-4 pb-8 pt-5 sm:px-8">
        {#if data.hledgerMissing}
          <div
            class="rounded-xl border border-rose-500/30 bg-rose-500/5 p-6 mb-6"
          >
            <h2 class="text-lg font-semibold text-rose-400 mb-2">
              hledger not found
            </h2>
            <p class="text-sm text-slate-100 mb-4">
              This app requires <a
                ...
                target="_blank">hledger</a
              > to be installed and available on your PATH.
            </p>
            <div
              class="rounded-lg bg-slate-800 p-4 font-mono text-sm text-slate-100 space-y-2"
            >
            ...
            <p class="text-sm text-slate-100 mt-4">
              Once installed, restart the dev server.
            </p>
          </div>
        {/if}
        {@render children()}
...
<Dialog.Root bind:open={showHelp}>
  <Dialog.Portal>
    ...
    <Dialog.Content
      class="fixed left-1/2 top-1/2 z-50 w-full max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-xl border border-slate-300 bg-slate-900 p-6 shadow-2xl"
    >
      <div class="mb-4 flex items-center justify-between">
        <Dialog.Title class="text-base font-semibold text-slate-100"
          >Keyboard Shortcuts</Dialog.Title
        >
        <Dialog.Close class="btn-close h-8 w-8 rounded-lg text-lg"
          >×</Dialog.Close
        >
      </div>
      <Dialog.Description class="sr-only"
        >List of keyboard shortcuts for navigating the application</Dialog.Description
      >
      <div class="space-y-1 text-sm">
        {#each [{ key: "n", label: "New transaction", href: "/add" }, { key: "/", label: "Focus search" }, { key: "?", label: "Toggle this help" }, { key: "Esc", label: "Close modal" }] as item}
          <div class="flex items-center justify-between gap-4 px-2 py-1.5">
            <span class="text-slate-100">{item.label}</span>
            <kbd
              class="rounded border border-slate-300 bg-slate-800 px-2 py-0.5 font-mono text-xs text-slate-100"
              >{item.key}</kbd
            >
          </div>
        {/each}
  ...
  <Dialog.Portal>
    ...
    >
      <div class="mb-5 flex items-center justify-between">
        <Dialog.Title class="text-base font-semibold text-slate-100"
          >Accounting Glossary</Dialog.Title
        >
        <Dialog.Close class="btn-close h-8 w-8 rounded-lg text-lg"
          >×</Dialog.Close
        >
      ...
      <div class="space-y-4">
        {#each filteredGlossary() as { term, def }}
          <div
            class="rounded-lg border border-slate-400 bg-slate-800/40 px-4 py-3"
          >
      ...
      {#if filteredGlossary().length < glossaryTerms.length}
        <button
          onclick={() => {
            showAllGlossary = !showAllGlossary;
          }}
          class="mt-4 w-full text-center text-sm text-slate-100 hover:text-slate-100 transition-colors"
          >{showAllGlossary
            ? "Show fewer"
            : `Show all ${glossaryTerms.length} terms`}</button
        >
        {#if showAllGlossary}
          <div class="mt-4 space-y-4">
            {#each glossaryTerms.filter((g) => !filteredGlossary().includes(g)) as { term, def }}
              <div
                class="rounded-lg border border-slate-400/50 bg-slate-800/20 px-4 py-3"
              >
                <dt class="mb-1 text-sm font-semibold text-slate-100">
                  {term}
                </dt>
                <dd class="text-sm leading-relaxed text-slate-100">{def}</dd>
              </div>
